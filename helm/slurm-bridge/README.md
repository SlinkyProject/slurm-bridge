...
| sharedConfig.deviceProfiles | list | `[{"backend":{"gresName":"gpu","type":"indexed-gres"},"driver":"gpu.example.com","name":"gpu-example","selector":"device.driver == 'gpu.example.com'"},{"backend":{"gresName":"nic","type":"indexed-gres"},"driver":"dra.net","name":"dranet-rdma","selector":"device.driver == 'dra.net' && has(device.attributes['dra.net'].pciAddress) && has(device.attributes['dra.net'].rdma) && device.attributes['dra.net'].rdma == true"}]` | DRA DeviceProfiles recognized by Slurm Bridge. A DeviceClass resolves to a profile when its single CEL selector exactly matches `selector`. `name` becomes the Slurm GRES type and must remain stable while allocations using the profile exist. `driver` must be a Kubernetes DRA driver name and `backend.gresName` must be a DNS-1123 label of at most 60 characters. |
